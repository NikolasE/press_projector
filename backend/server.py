...
def save_calibration():
    ...
    try:
        ...
        required_fields = ['source_points', 'destination_points', 'press_width_mm', 'press_height_mm']
...
@app.route('/api/calibration/validate', methods=['GET'])
def validate_calibration():
    """Validate calibration quality."""
    try:
        # Always reload calibration from DB to avoid stale in-memory state
        calibration_data = db.load_calibration()
        if not calibration_data:
            return jsonify({'error': 'No calibration data found'}), 404
        if not calibrator.load_calibration_data(calibration_data):
            return jsonify({'error': 'No calibration data found'}), 404
...
def handle_show_validation_pattern():
    ...
    try:
        # Enable boundary pattern to show warped rectangle
    ...
    try:
        ...
        if not debug_bypass_warp and calibrator.is_calibrated():
            try:
                ...
                warped = cv2.warpPerspective(img, H_inv, (out_w, out_h), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_CONSTANT)
            except Exception as e:
                print(f"Warp failed ({e}); resizing raw image")
